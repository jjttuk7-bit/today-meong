...
// Fallback logic for when OpenAI API is unavailable or throws an error
function getFallbackParams(theme: string, moodQuick: string, moodText: string, lang: string = "ko") {
...
app.post("/api/analyze-mood", async (req, res) => {
  ...
  if (!apiKey || apiKey === "MY_OPENAI_API_KEY" || apiKey === "") {
    ...
    const fallback = getFallbackParams(theme, moodQuick, moodText, lang);
  ...
  try {
    ...
    // Validate structure slightly to avoid any parsing failures down the road
    if (
